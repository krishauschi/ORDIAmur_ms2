...
fit_SFM <- lmer(SFM ~ Soil*Genotype*Treat + (1|Table) + (1|Block), data=dat)
summary(fit_SFM)
...
inter3_means <- data.frame(emmeans(fit_SFM, specs=~Soil:Genotype:Treat))
...
plot_SFM_3 <- ggplot(dat, aes(x = Treat, y = SFM)) +
  theme_bw() +
  geom_violin(
...
ggsave("SFM_Means_violin_final.jpg", 
       width=16,
       height=11,
       units="cm",
       dpi=500)
