...
MDS_Bray_psO_WP3_16S_raref_sqrt<-ordinate(psO_WP3_16S_raref, "MDS","bray", autotransform=TRUE)
#Print stress data, dimensions and number of tries
head(MDS_Bray_psO_WP3_16S_raref_sqrt)
#Create a MDS plot 
plot_MDS_Bray_psO_WP3_16S_raref_sqrt<-plot_ordination(psO_WP3_16S_raref, MDS_Bray_psO_WP3_16S_raref_sqrt, type="Sample", shape="rootstock", color="site") + 
  geom_point(size=3) +
...
plot_MDS_Bray_psO_WP3_16S_raref_sqrt
ggsave("PCA_Bray_WP3_16S_raref_sqrt-count_ellipse.png", width = 17, height = 12, units = "cm",dpi = 300)

# permanova on sqrt-count data
metadata <- as(sample_data(psO_WP3_16S_raref), "data.frame")
...
adonis2(distance(psO_WP3_16S_raref, method = "bray", autotransform = TRUE) ~ rootstock * site,
        data = metadata,
        permutations = 10000,
        by= "terms",             
...
#bray_dist <- distance(psO_WP3_16S_raref, method = "bray", autotransform = TRUE)
...
psO_WP3_16S_raref.dist <- phyloseq::distance(psO_WP3_16S_raref, method="bray", autotransform=TRUE)
#data Frame
df_WP3_16S_raref <- as(sample_data(psO_WP3_16S_raref), "data.frame")
#create combined strata-variable
df_WP3_16S_raref$meta_combined <- interaction(df_WP3_16S_raref$table, df_WP3_16S_raref$block)

#unique groups for pairwise genotype comparisons
unique_rootstocks <- unique(df_WP3_16S_raref$rootstock)      
#initialize results
...
for (i in 1:(length(unique_rootstocks) - 1)) {
  for (j in (i + 1):length(unique_rootstocks)) {
    ...
    selected <- df_WP3_16S_raref$rootstock %in% c(group1, group2)
    dist_subset <- as.dist(as.matrix(psO_WP3_16S_raref.dist)[selected, selected])
    metadata_subset <- df_WP3_16S_raref[selected, ]
...
unique_sites <- unique(df_WP3_16S_raref$site)      
